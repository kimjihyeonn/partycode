...
LAST_DAY(TO_DATE(:V_ET,'YYYYMM'))-TO_DATE(:V_ST,'YYYYMM')+1) B

WHERE A.M_ID=B.M_ID AND MOD(EXTRACT(MONTH FROM B.M_ID),2)=:EVENODD) C
...
WHERE A.M_ID=B.M_ID AND MOD(EXTRACT(MONTH FROM B.M_ID),2)=:EVENODD;
