...
public class MoviePatternEx {
	...
	String[] reply = {"<b>닥터 스트레인지</b>,마법계의 어벤져스 <영화리뷰 474번째 이야기> 영제: Doctor Strange(2016) 장르: 어드벤처,경이 런타임: 115분 관람 장소: 일산 롯데시네마 감독: 스콧 데릭슨 출연: 베네딕트 컴버베치, 레이첼...",
	 ...
	 //영화 장르 초기화 => 영화 장르 14가지
	String[] feel = {"액션","모험","어드벤처","스릴러","판타지","공포","경이","드라마","멜로","코미디","다큐","애니메이션","뮤지컬"};
	
	
	// 영화 장르를 저장할 배열 객체 생성
	int[] result = new int[feel.length]; //0으로 자동 초기화
	...
	for(int i=0;i<reply.length;i++) {
		...
		for(int j=0;j<feel.length;j++) {
			//compile(String regex) : 주어진 정규 표현식 으로 부터 페턴을 만드는 메소드
			Pattern p = Pattern.compile(feel[j]);
			
			//matcher(CharSequence input) : 댓글에서 패턴을 찾는 Matcher객체를 만드는 메소드
			Matcher m = Pattern.compile(feel[j]).matcher(reply[i]);
			
			int count =0;
			//find() : 패턴이 일치하면 true를 리턴하고, m 위치로 이동하는 메소드
